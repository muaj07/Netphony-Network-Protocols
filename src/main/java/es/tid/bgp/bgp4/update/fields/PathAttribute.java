...
public abstract class PathAttribute extends BGP4Object {
	...
	public PathAttribute(byte []bytes, int offset){
		...
		if (extendedLengthBit){//Attribute Length is two octets when extendedLengthBit Flag is set to 1
			mandatoryLength=4;
			pathAttributeLength=((((int)bytes[offset+2]&0xFF)<<8)& 0xFF00) |  ((int)bytes[offset+3] & 0xFF);
		}
		else //It is one octet when extendedLengthBit Flag is set to 0
			pathAttributeLength =  bytes[offset+2]&0xFF; //Actual length of the Attribute
